-- Migration 031: Three-Role System Phase 1 - Preparation
...
RETURNS TABLE(
    ...
    suggested_new_role user_role_new,
...
BEGIN
    ...
    SELECT 
        ...
        CASE 
            ...
            WHEN e.role = 'admin' AND e.email LIKE '%admin%' THEN 'admin'::user_role_new
            -- Developer organization users with admin role -> developer
            WHEN e.role = 'admin' AND o.type = 'developer' THEN 'developer'::user_role_new
            -- Agent organization users -> agent
            WHEN e.role = 'agent' AND o.type = 'agent' THEN 'agent'::user_role_new
            -- Fallback cases
            WHEN e.role = 'manager' AND o.type = 'developer' THEN 'developer'::user_role_new
            WHEN e.role = 'staff' AND o.type = 'developer' THEN 'developer'::user_role_new
            WHEN e.role = 'manager' AND o.type = 'agent' THEN 'agent'::user_role_new
            WHEN e.role = 'staff' AND o.type = 'agent' THEN 'agent'::user_role_new
...
INSERT INTO activity_logs (
    ...
    'system',
    NULL,
    '{"migration": "031_three_role_system_phase1", "phase": "preparation", "timestamp": "' || NOW() || '"}',
    NOW()
);
